 ...
 */
package com.frame.camera.sound;
...
public class SoundPlayer implements SoundPool.OnLoadCompleteListener {
    private static final String TAG = "SoundPlayer:Intelli";
    ...
    public void play(int resourceId, float volume) {
        ...
        if (mSoundIDToPlay == UNLOAD_SOUND_ID) {
            ...
            Log.d(TAG, "sound id " + mSoundIDToPlay + " is in loading and not ready yet");
        } else {
            mSoundPool
    ...
    public void onLoadComplete(SoundPool pool, int soundID, int status) {
        if (status != 0) {
            Log.d(TAG, "onLoadComplete : " + soundID + " load failed , status is " + status);
            return;
        }
